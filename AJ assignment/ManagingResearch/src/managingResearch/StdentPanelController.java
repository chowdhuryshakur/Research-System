...
public class StdentPanelController implements Initializable {
   ...
   private final String DB_URL = "jdbc:mysql://"+DB_HOSTNAME+"/"+DB_DBNAME;
    ...
    @FXML
    private void handleNotificationAction(ActionEvent event) throws IOException {
         AnchorPane pane =FXMLLoader.load(getClass().getResource("Notification.fxml"));
                        studentPanel.getChildren().setAll(pane);
    }

    @FXML
    private void handleApplyForResearchButtonAction(ActionEvent event) throws IOException {
         AnchorPane pane =FXMLLoader.load(getClass().getResource("studentApply.fxml"));
                        studentPanel.getChildren().setAll(pane);
    }
